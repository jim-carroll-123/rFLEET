...
interface Props {
  ...
  onSubmit: SubmitHandler<OceanLoadTypeInputs>
}
...
export const OceanLoadType = ({ methods, onSubmit }: Props) => {
  ...
  const onAdd = async () => {
    ...
    if (validated) {
      append({ ...initialOceanField })
    }
    return
  }

  const onFclAdd = async () => {
    const validated = await trigger('fclFields')
    if (validated) {
      fclAppend({ ...initialOceanFclField })
    }
    return
  }
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col lg:gap-[24px] gap-[18px]">
      ...
      <div className="relative rounded-d-8 p-3 border border-[#FFFFFF30]   w-full ">
        ...
        {watch('containerLoadType') == 'FCL' && (
          <div>
            {fclFields.map((item, index) => (
              <div key={index} className="grid lg:grid-cols-10 grid-cols-1  gap-4 my-4">
                <div className="col-span-3 flex flex-col gap-2">
                  <div className="text-sm">Container Type </div>
                  <Controller
                    name={`fclFields.${index}.containerType`}
                    control={control}
                    defaultValue=""
                    render={({ field }) => (
                      <Select
                        ...
                        error={errors.fclFields && errors.fclFields[index]?.containerType?.message}
                        containerClassName="w-full"
                        {...field}
                        value={findOption(containerTypes, watch(`fclFields.${index}.containerType`))}
                        onChange={({ value }) =>
                          setValue(`fclFields.${index}.containerType`, value, { shouldValidate: true })
                        }
                      />
                    )}
                  />
                ...
                <div className="col-span-5 flex flex-col gap-1">
                  <div className="text-sm flex items-center gap-5 ">
                    ...
                    <Check
                      ...
                      labelClassName="text-xs my-auto"
                      checked={watch(`fclFields.${index}.addExtraPickups`)}
                      onChange={(checked) => setValue(`fclFields.${index}.addExtraPickups`, checked)}
                    />
                  </div>

                  <div className="flex items-center gap-2">
                    <Controller
                      name={`fclFields.${index}.weight`}
                      control={control}
                      render={({ field }) => (
                        <Input
                          placeholder="Weight"
                    ...
                    />
                    <Controller
                      name={`fclFields.${index}.weightUnit`}
                      control={control}
                      defaultValue=""
                      render={({ field }) => (
                        <Select
                          ...
                          onChange={({ value }) =>
                            setValue(`fclFields.${index}.weightUnit`, value, { shouldValidate: true })
                          }
                        />
                      )}
                    />
                    ...
                    <Controller
                      name={`fclFields.${index}.weightType`}
                      control={control}
                      defaultValue=""
                      render={({ field }) => (
                        <Select
                          placeholder=""
                          options={weightTypes}
                          error={errors.fclFields && errors.fclFields[index]?.weightType?.message}
                          containerClassName="lg:w-[190px]"
                          {...field}
                          value={findOption(weightTypes, watch(`fclFields.${index}.weightType`))}
                          onChange={({ value }) =>
                            setValue(`fclFields.${index}.weightType`, value, { shouldValidate: true })
                          }
                        />
                      )}
                    />
